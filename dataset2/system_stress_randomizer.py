...
IPERF_SERVER = "10.0.20.57"  # replace with your iperf3 server IP
...
def apply_stress_for_slot(slot_num):
    ...
    # --- Network Upload Test (first half) ---
    print(f"  🌐 Starting UPLOAD test ({half_slot}s)...")
    iperf_tx_cmd = (
        f"iperf3 -u -c {IPERF_SERVER} -b {config['net_bandwidth']} "
        f"-t {half_slot} > /tmp/iperf_tx_slot{slot_num}.log 2>&1"
    )
    iperf_tx_proc = run_background(iperf_tx_cmd)
    iperf_tx_proc.wait()  # wait until upload completes

    # --- Small gap between upload and download ---
    time.sleep(1)

    # --- Network Download Test (second half) ---
    print(f"  🌐 Starting DOWNLOAD test ({half_slot}s)...")
    iperf_rx_cmd = (
        f"iperf3 -u -c {IPERF_SERVER} -R -b {config['net_bandwidth']} "
        f"-t {half_slot} > /tmp/iperf_rx_slot{slot_num}.log 2>&1"
    )
    iperf_rx_proc = run_background(iperf_rx_cmd)
    iperf_rx_proc.wait()  # wait until download completes

    # --- Stop all processes ---
    stop_process(stress_proc)

    print(f"[Slot {slot_num}] Completed {level.upper()} stress cycle.")
    print(f"  Logs: /tmp/iperf_tx_slot{slot_num}.log (upload), /tmp/iperf_rx_slot{slot_num}.log (download)")
...
if __name__ == "__main__":
    print("=== Random Stress Orchestrator Started ===")
